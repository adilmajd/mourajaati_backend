...
def get_user_niveau_r(user_public_id: str, session: Session = Depends(get_session),user = Depends(require_permission("read_level"))):
    return get_user_niveau(user_public_id,session)


@router.get("/cycles/{cycle_id}/niveaux", response_model=List[NiveauRead])
def get_niveaux_by_cycle_r(cycle_id: int, session: Session = Depends(get_session),user = Depends(require_permission("read_level"))):
    return get_niveaux_by_cycle(cycle_id,session)
   

@router.put("/user/{user_public_id}/niveau")
def update_user_niveau_r(user_public_id: str, payload: UpdateUserNiveau, session: Session = Depends(get_session),user = Depends(require_permission("update_level"))):
...
def list_niveaux(cycle: Session = Depends(get_session),user = Depends(require_permission("read_cycle"))):
